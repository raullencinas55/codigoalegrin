...
class B_AI:
    ...
    def move(hex_map: HexMap) -> tuple[HexCoord, HexCoord]:
        ...
        for (start, end) in hex_map.moves_for_col("b"):
            ...
            result: float = B_AI.minimax(hex_map, 4, -math.inf, math.inf, False)
    ...
    def minimax(hex_map: HexMap, depth: int, alpha: float, beta: float, maximising: bool) -> float:
        ...
        if state_hash in B_AI.cache:
            return B_AI.cache[state_hash]

        # Add a limit on how far down to search.
        if depth == 0:
            return B_AI.evaluate(hex_map)
        ...
        def capture_score(move: tuple[HexCoord, HexCoord]) -> int:
            return B_AI.capture_values[hex_map[move[0]]] + B_AI.capture_values[hex_map[move[1]]]

        moves = hex_map.moves_for_col("b" if maximising else "w" if maximising else "r")
        ...
        for (start, end) in moves:
            ...
            prev_state = hex_map[end]
            hex_map.make_move(start, end)
            result: float = B_AI.minimax(hex_map, depth - 1, alpha, beta, not maximising)
            hex_map.make_move(end, start)
            hex_map[end] = prev_state
        ...
        B_AI.cache[state_hash] = final_score
        return final_score

    @staticmethod
    def evaluate(hex_map: HexMap) -> float:
        map_to_vals = lambda cell: B_AI.capture_values[cell.state]
        ...
        return -(sum_vals("b") + sum_vals("w") +sum_vals("r")) + modifier
